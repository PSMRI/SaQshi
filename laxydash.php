...
$chcTotal = mysqli_fetch_assoc(mysqli_query($con, "SELECT COUNT(*) AS cnt FROM facilities WHERE Health_facilty_type = 1"))['cnt'];

$dhMusqanCount = mysqli_fetch_assoc(mysqli_query($con, "
    SELECT COUNT(DISTINCT facid) AS cnt 
    FROM department_wise_state_dash 
    WHERE Health_facilty_type = 2 AND fac_dept_id_fk IN (33, 9)
"))['cnt'];

$sdhMusqanCount = mysqli_fetch_assoc(mysqli_query($con, "
    SELECT COUNT(DISTINCT facid) AS cnt 
    FROM department_wise_state_dash 
    WHERE Health_facilty_type = 10 AND fac_dept_id_fk IN (33, 9)
"))['cnt'];

$chcMusqanCount = mysqli_fetch_assoc(mysqli_query($con, "
    SELECT COUNT(DISTINCT facid) AS cnt 
    FROM department_wise_state_dash 
    WHERE Health_facilty_type = 1 AND fac_dept_id_fk IN (3, 39)
"))['cnt'];
