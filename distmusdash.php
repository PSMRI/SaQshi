...
$dhMusqanCount = mysqli_fetch_assoc(mysqli_query($con, "
    ...
    WHERE fac_dept_id_fk IN (5,6,7,23) AND Health_facilty_type = 2 and dist_id=$distid
"))['cnt'];

$sdhMusqanCount = mysqli_fetch_assoc(mysqli_query($con, "
    SELECT COUNT(DISTINCT facid) AS cnt 
    FROM department_wise_state_dash 
    WHERE fac_dept_id_fk IN (5,6,7,23) AND Health_facilty_type = 10 and dist_id=$distid
"))['cnt'];
...
$query = "
    ...
    WHERE fac_dept_id_fk IN (5,6,7,23)
      AND Health_facilty_type IN (2, 10) and dist_id=$distid
