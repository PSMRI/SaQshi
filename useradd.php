...
<script>
    ...
    if(!distId){
        ...
        $("#facility").html("<option value='0'>- Select Facility -</option>");
        return;
    }
    ...
    $.post(
        ...
        function(data){
            ...
            // 🔥 Select first VALID block (value not empty)
            let firstBlock = $("#block option").filter(function(){
                return $(this).val() !== "";
            }).first().val();

            if(firstBlock){
                $("#block").val(firstBlock);
   ...
   $("#block").change(function(){
    ...
    $.post(
        ...
        function(data){
            ...
            let firstFacility = $("#facility option").filter(function(){
                return $(this).val() !== "";
            }).first().val();

            if(firstFacility){
                $("#facility").val(firstFacility);
        ...
        $.ajax({
            ...
            success: function(d) {
                ...
                if (d.nin_no && d.nin_no !== "0") {
                    ...
                    $("#createBtn").prop("disabled", true);
                    alert("⚠ Facility has no valid NIN.");
                }
            },
            error: function() {
                $("#createBtn").prop("disabled", true);
