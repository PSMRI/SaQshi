...
function loadFacilityList(){
    $.get("assets/get/chat_backend.php",{ load_facilities:1 },function(d){
        $("#facilityList").html(d);
    });
}
loadFacilityList();
...
function loadMessages(scrollDown=false){
    ...
    $.get("assets/get/chat_backend.php",{
        ...
        let items = temp.find(".message-item");
...
$("#sendMessageForm").on("submit",function(e){
    e.preventDefault();
    if(isSending || !activeFacility) return;
...
$("#fileInput").change(function(){
    if(!this.files.length || !activeFacility) return;
