    ...
    <div class="pcoded-content">
        ...
        if (isset($_POST['submit1'])) {
            ...
            if ($new_date1 == '1970-01') {
                ...
                $check_query = "SELECT COUNT(phc_kpi_id) AS total FROM phc_kpi_in WHERE phc_kpi_date = ? AND phc_kpi_fac_id = ?";
        ...
        <form method='post'>
            ...
            <button type='submit' 
                ...
                <i class='bi bi-pencil-square'></i> View / Edit
                    ...
                    $q3 = mysqli_query($con, "SELECT * FROM phckpi");
                    $q = 0;
                    ...
                    while ($row = mysqli_fetch_array($q3)) {
                        $q++;
                        $readonly = ($row['phc_kpi_d'] === 'N/A') ? 'readonly' : '';
                        $isLast = ($q == $total);
                        $inputNumId = $q * 2 - 1;
                        $inputDenId = $q * 2;
                        ...
                        <div class='card-body'>
                            <h6 class='card-title text-primary'><i class='bi bi-bar-chart-fill me-2'></i>KPI Indicator: {$row['phc_kpitext']}</h6>
                            <div class='alert alert-primary py-2 px-3 small d-flex align-items-center'>
                                <i class='bi bi-info-circle-fill me-2'></i>
                                <div><strong>Expected:</strong> <span class='text-warning'><b>Numerator:</b></span> <b>{$row['phc_kpi_n']}</b>,  <span class='text-warning'><b>Denominator:</b></span> <b>{$row['phc_kpi_d']}</b></div>
                            </div>
                            <div class='row g-2'>";
                    ...
                    <input type='hidden' name='out_come_id$q' value='{$row['phc_kpi_id']}'>
                    ...
                    echo "</form>";
                    echo "<script src='assets/calculationjs/phckpi.js'></script>";
                }
            }
        }
        ?>
...
SELECT
    ...
    COALESCE(i.phc_kpi_value,0) AS phc_kpi_value,
    COALESCE(i.phc_kpi_n,0) AS phc_kpi_num,
    COALESCE(i.phc_kpi_d,0) AS phc_kpi_deno

FROM phckpi k

LEFT JOIN phc_kpi_in i
ON k.phc_kpi_id = i.phc_kpi_id
AND i.phc_kpi_date = '$date1'
AND i.phc_kpi_fac_id = '$fac_id'
";
...
value='$date1'>";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $value = $row['phc_kpi_value'] ?? '';

                $num = $row['phc_kpi_num'] ?? '';

                $den = $row['phc_kpi_deno'] ?? '';
...
KPI updated successfully
        ...
        if (isset($_POST['postsubmit3'])) {
            ...
            $result = mysqli_query($con, "SELECT phc_kpi_id FROM phckpi");
            $success = true;
            $errorMessages = [];
            $index = 0;
