...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['postsubmit'])) {
    ...
    $required = [
        'dist','block_id','fac_id','fac_name','fac_type',
        'cert_type','cert_detailscol','cert_status','ass_mod',
        'date_of_ass','cert_issue','score','lat','longi'
    ];
    ...
    if ($errorMsg === "") {
        ...
        $stmt->bind_param(
            $types,
            $dist, $dist_name, $block_id, $block_name, $fac_id, $fac_nin,
            $fac_name, $fac_type, $cert_type, $cert_detailscol, $cert_issue,
            $validity, $score, $lat, $longi, $cert_status, $ass_mod,
            $date_of_ass
        );
...
?>
<style>
.glow-card { background:#fff; border-radius:10px; box-shadow:0 0 12px rgba(0,123,255,0.15); }
</style>
...
function setFacilityData() {
    ...
    document.getElementById("fac_nin").value = sel.getAttribute("data-nin");
}
